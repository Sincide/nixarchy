...
# Users can still opt-in to Cloudflare DNS using: archy-setup-dns cloudflare
...
if [ -f /etc/systemd/resolved.conf ]; then
  ...
  if [ -f /etc/systemd/network/99-archy-dns.network ]; then
    sudo rm -f /etc/systemd/network/99-archy-dns.network
  ...
  echo "To use Cloudflare DNS, run: archy-setup-dns Cloudflare"
fi
