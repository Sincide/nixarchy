...
# Users can still opt-in to Cloudflare DNS using: nixarchy-setup-dns cloudflare
...
if [ -f /etc/systemd/resolved.conf ]; then
  ...
  if [ -f /etc/systemd/network/99-nixarchy-dns.network ]; then
    sudo rm -f /etc/systemd/network/99-nixarchy-dns.network
  ...
  echo "To use Cloudflare DNS, run: nixarchy-setup-dns Cloudflare"
fi
