...
if [ "$(plymouth-set-default-theme)" != "nixarchy" ]; then
  sudo cp -r "$HOME/nixarchy/default/plymouth" /usr/share/plymouth/themes/nixarchy/
  sudo plymouth-set-default-theme -R nixarchy
fi
...
if [ ! -f /etc/systemd/system/nixarchy-seamless-login.service ]; then
  cat <<EOF | sudo tee /etc/systemd/system/nixarchy-seamless-login.service
[Unit]
Description=Nixarchy Seamless Auto-Login
Documentation=https://github.com/basecamp/nixarchy
...
# Enable nixarchy-seamless-login.service only if not already enabled
if ! systemctl is-enabled nixarchy-seamless-login.service | grep -q enabled; then
  sudo systemctl enable nixarchy-seamless-login.service
fi
